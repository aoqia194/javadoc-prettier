...
function replaceTokens(node, tokens, type) {
  ...
  tokens.forEach((token) => {
    ...
    if (token.type === "keyword" /* KEYWORD */) {
      ...
      fragment.appendChild(span);
    } else if (token.type === "field" /* FIELD */) {
      const span = document.createElement("span");
      span.textContent = token.value;
      ...
      if (parent instanceof HTMLAnchorElement) {
        span.classList.add(CSS_CLASSES.theme.class);
      } else if (parent.classList.contains("parameters") || parentParent.classList.contains("method-summary-table") || parent.firstElementChild?.classList.contains("member-name-link")) {
        ...
        if (last.type !== "syntax" /* SYNTAX */ && !last.value.endsWith(")")) {
          span.classList.add(CSS_CLASSES.theme.fieldName);
        } else {
...
function parseSummaryTable(id, e) {
  ...
  if (isThreeColumn) {
    const second = id === IDS.nestedClassSummary || id === IDS.fieldSummary ? parseSummaryTableName : parseSummaryTableSignature;
